const { db } = require("../../firebase") // adjust path depending on where firebase.js is
...
module.exports = async (req, res) => {
    ...
    const allowedOrigins = process.env.ALLOWED_ORIGIN
        ? process.env.ALLOWED_ORIGIN.split(",") // make it an array
        : []
    ...
    if (req.method === "POST" && req.url.includes("/api/initialize-transaction")) {
        try {
    ...
    // --- Verify payment ---
    else if (req.method === "POST" && req.url.includes("/api/verify-payment")) {
        try {
